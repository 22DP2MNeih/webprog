...
function GetPartIndexes($part) {
    ...
    $stmt = $mysqli->prepare(
        "SELECT"
	    + "part_display_colums.Display_columns,"
	    + "CONCAT(display_colums.Data_type, part_display_colums.Column_id) AS `Column`"
        + "FROM part_display_colums"
        + "INNER JOIN display_colums ON part_display_colums.Display_columns = display_colums.id"
        + "WHERE Part_type = ?;"
    );
